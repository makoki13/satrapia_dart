...
class Cuartel extends Edificio {
  ...
  Capital _capital;
  Dispatcher _disp;
  late Ejercito _ejercito;

  late UnidadMilitar _tipoUnidad, _tipoUnidadCivilConHonda;
  late int _cantidad, _cantidadCivilConHonda;

  Cuartel(this.id, this.nombre, this._capital, this._disp)
      : super(id, nombre, TipoEdificio.CUARTEL, _capital.getPosicion(), 0, 0) {
    this._capital.setCuartel(this);
    this._unidades = [];
    this._ejercito = new Ejercito(
        1,
        'Ejercito de la ciudad de ${this._capital.getNombre()}',
        _capital.getPosicion(),
        this._capital,
        this._disp);
  }

  List<Unidades> getTropas() {
  ...
  void transfiere(UnidadMilitar tipo, int cantidad) {
    ...
    if (indiceElemento != -1) {
      print(
          "${tipo.getNombre()} : ${this._unidades[indiceElemento].cantidad} : $cantidad");
      if (this._unidades[indiceElemento].cantidad <= cantidad) {
        this
            ._ejercito
            .addUnidades(tipo, this._unidades[indiceElemento].cantidad);
  ...
  addUnidades() {
    UnidadMilitar tipo = this._tipoUnidad;
  ...
  addUnidadesCivilesConHonda() {
    UnidadMilitar tipo = _tipoUnidadCivilConHonda;
    int cantidad = this._cantidadCivilConHonda;
    int indiceElemento = -1;
    int indice = 0;
    this._unidades.forEach((x) {
      if (x.unidad.getID() == tipo.getID()) {
    ...
    myCI.getListaUnidadesMilitaresConseguidas().forEach((item) {
      ...
      switch (indice) {
        case 1:
          ...
          if (existe == false) {
            Unidades nuevaUnidad =
                new Unidades(new CivilConHonda(100, 1, 1, 100), 0);
            this._unidades.add(nuevaUnidad);
        ...
        case 2:
          ...
          if (existe == false) {
            Unidades nuevaUnidad =
                new Unidades(new Soldado(100, 1, 100, 100), 0);
            this._unidades.add(nuevaUnidad);
            lista.add(nuevaUnidad);
          }
          break;
  ...
  int _entrenaCivilesConHonda([int cantidad = 100]) {
    ...
    if (cantidad == 0) {
      cantidad = maxUnidadesEnEntrenamiento;
    }
    if (cantidad > maxUnidadesEnEntrenamiento) {
      cantidad = maxUnidadesEnEntrenamiento;
    }
    CentroDeInvestigacion myCI = this._capital.getCentroDeInvestigacion();
  ...
  int _entrenaSoldados([int cantidad = 0]) {
    ...
    if (myCI.estaComprada(2, 1, 2)) {
      ...
      print(
          ' No se puede entrenar "Soldados". La investigación no está realizada.');
    }
